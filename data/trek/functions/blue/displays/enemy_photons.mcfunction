execute if entity @p[nbt={Inventory:[{tag:{Energy:1b}}]}] as @e[type=text_display,tag=blue_espionage] run data merge entity @s {text:'["Photons: ",{"color":"red","score":{"name":"red","objective":"photon"}}]'}
